...
)
{
    ...
    ** 找到用于存储内存块的内存cell,需要VdbeCursor结构体,使用vdbe memory cell来管理内存分配是很方便的.
    **
    **   * Sometimes cursor numbers are used for a couple of different
    **     purposes in a vdbe program. The different uses might require
    **     different sized allocations. Memory cells provide growable
    **     allocations.
    **   * 有些时候,游标number用于一系列的目的,不同的目的可能需要不同大小的内存分配,Memory cell提供可以增长的分配.
    **
    ...
    */
    ...
    if (p->apCsr[iCur])
    {
        sqlite3VdbeFreeCursor(p, p->apCsr[iCur]); /*   释放之前的cursor */
        p->apCsr[iCur] = 0;
    }
...
**    尝试将pRec转换为整数或者浮点数表示,如果整数表示是可能的话.我们总是倾向于表示为整数
**    即使affinity为REAL,因为整数表示在磁盘上更加高效
**
** SQLITE_AFF_TEXT:
...
)
{
    if (affinity == SQLITE_AFF_TEXT)
    {
        ...
        */
        ...
        {
            sqlite3VdbeMemStringify(pRec, enc); /* 转换为字符串表示 */
        }
...
** 尽可能多地执行VDBE程序,在我们返回之前.
**
** sqlite3VdbeMakeReady() must be called before this routine in order to
** close the program with a final OP_Halt and to set up the callbacks
** and the error message pointer.
** sqlite3VdbeMakeReady()必须在这个程序之前调用,为了以一个最终的OP_Halt结束这个程序.
** 设置好回调函数,和错误消息指针.
**
** Whenever a row or result data is available, this routine will either
** invoke the result callback (if there is one) or return with
** SQLITE_ROW.
** 当成功获取到一列或者数据结构,这个函数要么执行函数回调,要么返回SQLITE_ROW
**
** If an attempt is made to open a locked database, then this routine
** will either invoke the busy callback (if there is one) or it will
** return SQLITE_BUSY.
** 如果尝试打开被锁住的数据库,那么这个函数要么回调busy callback,要么返回SQLITE_BUSY.
**
** If an error occurs, an error message is written to memory obtained
** from sqlite3_malloc() and p->zErrMsg is made to point to that memory.
** The error code is stored in p->rc and this routine returns SQLITE_ERROR.
** 如果错误发生,一条错误信息将会被写入内存(p->zErrMsg指向),错误码将会存入p->pc,此函数
** 将会返回SQLITE_ERROR
**
** If the callback ever returns non-zero, then the program exits
** immediately.  There will be no error message but the p->rc field is
** set to SQLITE_ABORT and this routine will return SQLITE_ERROR.
** 如果回调的返回值是非空,虚拟程序立马停止,没有错误信息,但是p->pc被设置为SQLITE_ABORT
** 这个函数返回SQLITE_ERROR
**
...
** 执行完此程序之后,sqlite3VdbeFinalize()函数应该被调用,来做清理工作.
*/
int sqlite3VdbeExec(
    Vdbe *p                    /* The VDBE */
)
...
#endif
    for (pc = p->pc; rc == SQLITE_OK; pc++) /* 开始一条一条执行指令 */
    {
        ...
        ** 如果opcode带有out2-prerelease标记,释放mem[p2],设置mem[p2]为未定义的整数.
        **
        */
        ...
        switch (pOp->opcode) /* 接下来开始正式执行指令 */
        {
            ...
            ** 接下来是一个庞大的 switch 语句，其中每个 case 实现了一个虚拟机中的单独指令。 如果我们按照惯例
            ** 缩进约定，每个 case 应该缩进 6 个空格。 但这在左边距上浪费了很多空间。 所以里面的代码switch
            ** 语句将打破惯例并左对齐。 其他大注释（类似于这个）将标记代码中的点我们过渡回正常缩进。
            **
            ...
            */
            ...
            case OP_Remainder:             /* same as TK_REM, in1, in2, out3 */
            {
                ...
                if ((pIn1->flags & pIn2->flags & MEM_Int) == MEM_Int)
                {
                    ...
                    {
                        case OP_Add:
                            if (sqlite3AddInt64(&iB, iA)) goto fp_math;
                            break;
                        case OP_Subtract:
                            if (sqlite3SubInt64(&iB, iA)) goto fp_math;
                            break;
                        case OP_Multiply:
                            if (sqlite3MulInt64(&iB, iA)) goto fp_math;
                            break;
                        case OP_Divide:
                        {
            ...
            ** 将P1寄存器的值强制转换为int,如果当前的值是一个浮点数,去掉分数部分,如果值是text或者blob
            ** 使用atoi或者等价函数来转换,如果不可转换,直接存储0.
            **
            ** A NULL value is not changed by this routine.  It remains NULL.
            */
            ...
            ** 跳转到P2, 如果寄存器P1的值为true,如果P1为NULL,那么如果P3不为0,则跳转
            */
            ...
            ** 给数据库打上或者去掉auto-commit标记,(取决于P1的值),P2为true的话,回滚掉所有的事务.
            **
            ...
            */
            case OP_AutoCommit:
            {
                ...
                int iRollback;
                int turnOnAC; /* 是否要打开autocommit */

                desiredAutoCommit = pOp->p1;
                iRollback = pOp->p2; /* 是否要回滚 */
...
#   define MAX_ROWID  (i64)( (((u64)0x7fffffff)<<32) | (u64)0xffffffff )
            ...
            ** 读取以及解析SQLITE_MASTER表中满足where条件的所有entry,P1用于指示数据库
            ** P4用于指示where条件
            **
            ...
            ** 获取特定表的锁,这条指令只有当shared-cache特性启用的时候才会使用.
            **
            ** P1 is the index of the database in sqlite3.aDb[] of the database
            ** on which the lock is acquired.  A readlock is obtained if P3==0 or
            ** a write lock if P3==1.
            ** P1是数据库的索引值. P3为0,要加读锁,否则写锁.
            **
            ** P2 contains the root-page of the table to lock.
            ** P2包含了要加锁的表的root-page
            **
            ** P4 contains a pointer to the name of the table being locked. This is only
            ** used to generate an error message if the lock cannot be obtained.
            ** P4包含了一个指向被锁定表名的指针,这只是用于在无法获取锁的时候,生成错误信息.
            */
            case OP_TableLock:
            {
            ...
            ** 如果tracing功能开启,那么P4中UTF-8字符串在跟踪回调中发出(emit)
            */
            case OP_Trace:
            {
                char *zTrace; /* 字符串 */
