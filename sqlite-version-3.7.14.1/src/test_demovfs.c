...
)
{
    ...
    ofst = lseek(p->fd, iOfst, SEEK_SET); /* 偏移 */
    if (ofst != iOfst)
    {
        return SQLITE_IOERR_WRITE;
    }

    nWrite = write(p->fd, zBuf, iAmt); /* 写入 */
    if (nWrite != iAmt)
    {
        return SQLITE_IOERR_WRITE;
    }
...
)
{
    ...
    sqlite3_snprintf(nPathOut, zPathOut, "%s/%s", zDir, zPath); /* 全路径 */
